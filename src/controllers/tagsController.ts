...
controller.put(
    ...
    async (req: TokenRequest, res: Response, next: NextFunction) => {
        try {
            ...
            if (!task || task.projectId !== projectId) throw new ResponseException("Task not found!", 404)

            if (task.status === "done") throw new ResponseException("Cannot edit done tasks!", 401)

            const tagId = Number.parseInt(req.params.tagId)

            if (!hasTag(taskId, tagId)) throw new ResponseException("Task doesnt have tag!", 404)
